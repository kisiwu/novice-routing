...
function setMiddlewares(){
  var handles = flatten(Array.prototype.slice.call(arguments));
  this._middlewares = [].concat(handles);
  // add layer options
  handles.unshift({
    sensitive: this.caseSensitive,
    strict: this.strict,
  ...
  this.stack.forEach(
    routerLayer => {
      ...
      // registered with 'use' method
      if (routerLayer.route) {
        routerLayer.route.setMiddlewares.apply(routerLayer.route, handles);
      }
    }
  );
}
...
function addProperties(router) {
  ...
  // add '_middlewares' property to store handlers
  Object.defineProperty(router, '_middlewares', {
    value: [],
    writable: true
  });
  // add 'setMiddlewares' method (stores handlers into '_middlewares')
  Object.defineProperty(router, 'setMiddlewares', {
    value: setMiddlewares,
    writable: true
  });
}
